...
app.controller("ResultCtrl", function($scope, $window, $sce, $http, $localStorage, $location) {
  ...
  $scope.drawGraph = function(units) {
    ...
    for (var x=0; x<25; x++) {
      ...
      for (var z=1; z<41; z++) {
        if (units>0) {
          row.push('<td class="results_cell" style="border: 1px solid #777; width:10px !important; min-width:10px;"><img src="' + $scope.base_url + 'files/cellfill.png" title="filled cell" alt="filled cell" /></td>');
        }
        else {
          row.push('<td class="results_cell" style="border: 1px solid #777; width:10px !important; min-width:10px;"><img src="' + $scope.base_url + 'files/cellempty.png" title="empty cell" alt="empty cell" /></td>');
        };
        units-=1;
      };
      cellArray.push(row);
