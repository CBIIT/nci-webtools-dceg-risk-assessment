...
function processSubmission(form){
	...
	// 	},
	// 	data: { age:40, race: "White", age_period: 12, childbirth_age : 25, biopsy: 0, relatives: 1 },
	// 	beforeSend: function(jqXHR, settings) { docuemnt.write(settings.url); console.log(settings.url) }
	// }).done(resultsDisplay)
	// .fail(function() {
	// 	console.log("error");
	// })
...
/* gendar is on the page.  If it is on the page then male/female play a part  */
...
function isMaleOrFemaleChekced() {
	var result =  ( $("input[name='gender']").length == 0 ) ? true : result =  $("input[name='gender']:checked").length;
	return result;
}
...
$(function() {
	...
	$.each($("#riskForm section"), function(index, element) {
	  ...
	  $(navigationLinks).slice(startIndex, endIndex).attr('data-riskFormSection', index);
		...
		console.log("The value is " + heightOfFormSteps + currentTitleAndSecitonHeight);
