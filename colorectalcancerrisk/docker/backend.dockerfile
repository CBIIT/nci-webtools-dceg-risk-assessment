...
COPY colorectalcancerrisk/server/requirements.txt /app/server/requirements.txt

RUN pip3 install -r /app/server/requirements.txt

COPY colorectalcancerrisk/server /app/server

COPY colorectalcancerrisk/client /app/client
