...
namespace CCRAT
{
    ...
    public class QuestionControl : QuestionItemBase
    {
        ...
        private void RenderResultContents(HtmlTextWriter writer)
        {
            ...
            //convert back to feet and inches from inches(this is mainly for height)
            if (!string.IsNullOrEmpty(AnswerUnit) && AnswerUnit.Equals("FeetAndInches", StringComparison.OrdinalIgnoreCase))
            {
                writer.Write("{0}&nbsp;feet&nbsp;&nbsp;{1}&nbsp;inches", int.Parse(Answer) / 12, int.Parse(Answer) % 12);
            }
            else
            {
        ...
        private void RenderQuestionText(HtmlTextWriter writer)
        {
            ...
            if (!string.IsNullOrEmpty(controlID))
            {
                ...
                //add id for label
                writer.AddAttribute(HtmlTextWriterAttribute.Id, ID.Replace(ID.Substring(0, 2), "lbl"));
