...
namespace CCRAT.RiskCalculator
{
    public class Manager
    {
        ...
        /// <returns></returns>
        public static AbsoluteRisks CalculateRisks(string race
         , string currentAge
         , string gender
         , string height
         , string weight
         ...
         )
        {
            ...
            if (eGender == Genders.Male)
            {
                ...
                return RiskCalculator.CalculateAbsRiskForMen(iCurrentAge, Numrel, Cigarets, Duration, NoNSaids, NoIbuprofn, Sigmod, VegLt5, BMI_Trnd, HrsExcrise, bmi);
            }
            else
            {
                ...
                return RiskCalculator.CalculateAbsRiskForWomen(iCurrentAge, Numrel, Nostrogen, NoNSaids, Sigmod, VegLt5, BmiGe30, HrsExcrise, bmi);
            }
        }
        ...
        private static int CalculateSigmod(string colonoscopy, string hadPolyp)
        {
             ...
             * if colonoscopy= yes and hadPolyp = no          then    sigmod=0;
                if colonoscopy= yes and hadPolyp = I don’t know then    sigmod=0;
        ...
        private static int CalculateNoNSaids(string asprin, string ibuprofen)
        {
            ...
            if (asprin != "Yes" && asprin != "No")
                throw new Exception("Asprin has an unknown answer.");

            if (ibuprofen != "Yes" && ibuprofen != "No")
