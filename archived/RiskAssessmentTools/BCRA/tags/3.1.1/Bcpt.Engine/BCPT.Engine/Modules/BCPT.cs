/**************************************************************************************************
* Name		: BCPT.cs
* Purpose	: Calculates the absolute and average risks of breast cancer based on the BCPT model 
* Author	: SRamaiah
* Date		: 03/23/2006
* Note      : BCPT - (B)reast (C)ancer (P)revention (T)rial
* Changes	: 03/23/2006 - migrated from original FORTRAN code based on GAIL model
            : 11/27/2007 - the following changes are done
              1. new beta values for african american woman
              2. new 1-AR values for african american woman
              3. new bcra hazard (h1) and computing risk (h2) values for african american woman
              4. recoded agemen=2( age<12 ) to agemen=1 [12, 13] and reset AGEFLB to 0 for african american woman 
/**************************************************************************************************
* Description:  
    BCPT.for:  (B)reast (C)ancer (P)revention (T)rial
    calculates the absolute risk of breast cancer based on the BCPT model.
    DOES NOT perform confidence interval calculations.

    also
    calculates the absolute risk of breast cancer based on an "average
    woman" of the same age and race as user, useful for comparison purposes.
    Age specific baseline breast cancer composite incidence from SEER*Stat
    1992:1996 incidence rates.  Age specific baseline mortality rates
    (excluding death from breast cancer) from NCHS mortality rates 1992:1996.

    also
    calculates the absolute risk of breast cancer of a woman of the same age
    and race as the user but with zero risk factors.

    also
    performs projection for HISPANIC women as in the STAR trial. See e-mail
    M. Gail to J. Costantino dated May 12, 2000 12:48 pm cc D. Pee et al.
    Hispanic breast cancer baseline composite incidence (SEER 1990:96) and
    competing hazard (excluding breast cancer mortality US 1990:96) are
    obtained from a Word document included as an attachment to this e-mail.
    Telephone conversation with M. Gail (5/12/2000 3:30pm) for "average women"
    the conversion factor (1-attributable risk) is 1.0 i.e. attributable
    risk=0, while for the women at baseline, the conversion factor is what it
    should be i.e. attributable risk .5788413 for whites and hispanic .41453
    or .42275 for young African American (<50) and older African American (>=50) women respectively.

    program inputs:

    current age,  risk assesment age,  race  and  risk factors.


    current age:
              (a) 20 =< current age  <= 85            and
              (b) current age < risk assesment age    and
              (c) age menarche       <= current age   and
              (d) age 1st live birth <= current age

         note: exception for (c), age menarche = 99 (missing)
               exception for (d), age 1st live = 99 (missing)

              program aborts if current age does not
              meet the above criterions.


    risk assesment age:
                     (a) current age < risk assesment age <= 90

                     program aborts if risk assesment age does not
                     satisfy this inequality.

    age menarche:
               (a) (7 =< age menarche <= 39  or  unknown=99) and
               (b) age menarche <= current age               and
               (c) age menarche <= age 1st live birth

    note: even if (a) is satisfied, (b) and (c) must also be satisfied.
          exceptions for (b) and (c) age menarche = 99 (missing)
          exceptions for (c) age 1st live birth   =  0 (no live birth)

          program aborts if age menarche not coded as above.


    14 =< age menarche     : level=0     (includes age menarche=99 (unknown))
    12 =< age menarche < 14: level=1
    7 =< age menarche < 12: level=2



    age 1st live birth:
                     (a) 10 =< age 1st live <= 55   no live birth=0,
                                                    unknown=99.
                     (b) age 1st live birth <= current age
                     (c) age menarche =< age 1st live birth <= 55

           note: exception for (b) age 1st live birth = 99 (missing)
                 exception for (c) age menarche = 99 (unknown)
                                                or
                                   no live birth (age 1st live birth=0)

                     program aborts if age 1st live birth not as above.

       age 1st <  20 or age 1st=99: level=0
    20 =< age 1st <  25              : level=1
    25 =< age 1st <  30 or age 1st= 0: level=2
    30 =< age 1st <= 55              : level=3



    ever had biopsy: no=0,  yes=1,  unknown=99.


    # biopsies:
             (a) 0 =< # biopsies <= 30  or  unknown=99.
             (b) ever had biopsy =  no, then # biopsies must be     =  0.
             (c) ever had biopsy = yes, then # biopsies must not be =  0.
             (d) ever had biopsy = unk, then # biopsies must be     = 99.
             program will check for these conditions.
             if this condition is not satisfied program aborts.

    # biopsy=0 or (# biopsy=99  &  ever had biopsy=99):  level=0
    # biopsy=1 or (# biopsy=99  &  ever had biopsy= 1):  level=1
    2 =< # biopsy <= 30                               :  level=2



    atypical hyperplasia: no=0,  yes=1,  unknown=99.

                       note this questions must be asked when and only when:

                       (a) if 0 < # biopsies <= 30  &  ever had biopsy=yes
                                                  or
                       (b) if # biopsies=unknown   &  ever had biopsy=yes



    # 1st degree relatives with breast cancer: valid codes are 0 to 31,
                                            unknown=99.
                                            program aborts if not as thus.

      # rel =   0 or # rel =99: level=0
      # rel =   1             : level=1
    2 =< # rel <= 31             : level=2


    race: 1=white,  2=African American,  3=hispanic,  4=other.  program aborts if race
                                                  is not 1,2,3 or 4. i.e.
                                                  must record a valid race
**************************************************************************************************/
using System;
using System.Text;
using System.Xml;
namespace NCI.BCPT.Engine
{
