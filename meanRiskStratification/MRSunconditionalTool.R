...
MRSunconditional <- function(abcd) {
  ...
  output <- matrix(c(OR,PPV,(1-NPV),sens,spec,Youden,AUC,p,q,danger,reassurance,MRS,t,D,dangerstar,reassurancestar,rr,nnr,probDM,probDnotM,probnotDM,probnotDnotM,ORCI[1],PPVCI[1],cNPVCI[1],sensCI[1],specCI[1],YoudenCI[1],AUCCI[1],pCI[1],qCI[1],dangerCI[1],reassuranceCI[1],MRSCI[1],tCI[1],DCI[1],rep(NA,8),ORCI[2],PPVCI[2],cNPVCI[2],sensCI[2],specCI[2],YoudenCI[2],AUCCI[2],pCI[2],qCI[2],dangerCI[2],reassuranceCI[2],MRSCI[2],tCI[2],DCI[2],rep(NA,8)),nrow=length(rownames),ncol=3,byrow=F)
